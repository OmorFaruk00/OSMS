...
if(isset($_POST['signin'])){
  ...
  if(mysqli_num_rows($result) > 0 ){
    ...
    $sql1 = "INSERT INTO `user_ragistration`(`name`, `email`, `password`) VALUES ('{$name}','{$email}','{$pass}')";
   ...
   <div class="row mt-5 offset-4">
    <div class="col-md-6 ">
      <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" class="shadow-lg p-5 bg-white ">
        ...
        $_GET['alert_message'] = NULL; ?>  <script>setTimeout(function(){ window.location.href ="login.php"; }, 2000); </script><?php } ?>
      </form>
    </div>      
  </div>
</div>
